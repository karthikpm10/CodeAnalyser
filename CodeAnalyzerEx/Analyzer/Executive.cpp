...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
	...
	std::cout << "Specification of the XML results file(takes default if argument not specified\n";
	if (argc < 2)
  {
    showUsage();
    return false;
...
int main(int argc, char* argv[])
{
  ...
  std::cout << "\n***********************Requirement 9-Running automated test suite to demonstrate requirements**********************************\n";
  showUsage();
  try {
   ...
   // std::cout << "\n  Code Analysis completed";
	std::cout << "\n***********************Requirement 3-Created TypeTable package(TypeAnalysis),DependencyAnalyser Package,Strong Component Package,Display Package:**********************************\n";
	//type analysis object created
	TypeTable tb;
	tb.doTypeAnal();
	DependencyTable dp(exec.getDepXMLName());
	dp.buildDependencyTable(exec.getFiles(), tb);
	StronglyCC sc(exec.getSCCXMLName());
	sc.buildSCC(dp);
	Disp objdisp;

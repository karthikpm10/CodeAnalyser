...
void XmlParser::extractAttributes()
{
  ...
  {
    if (xmlParts[static_cast<unsigned int>(pos)] == "=")
    {
      auto item = attrib(xmlParts[static_cast<unsigned int>(pos) - 1], xmlParts[static_cast<unsigned int>(pos) + 1]);
...
XmlParser::sPtr XmlParser::createComment()
{
  ...
  {
    comment += xmlParts[static_cast<unsigned int>(i)];
